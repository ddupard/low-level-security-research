...
echo "-------------------------------------------------------"
echo "  LABORATOIRE DE TEST - COGNITIVE FIREWALL (Warsaw 2026)"
echo "-------------------------------------------------------"
...
echo "4) Kernel 5.0  (Modern Hardening)"
echo "5) Kernel 5.15 (LTS)"
echo "q) Quitter"
echo "-------------------------------------------------------"
...
case $choice in
    ...
    4)
        VERSION="5.0"
        KERNEL_PATH="./5.0/arch/x86/boot/bzImage"
        ;;
    5)
...
VERSION="5.15"
        ...
        LKM_PATH="./LKM/cf_observer.ko"
        LKM2_PATH="./LKM2/my_attack.ko"
        
        if [ -f "$LKM_PATH" ]; then
            echo "Préparation de l'initramfs pour le Kernel 5.15..."
...
if [ -f /cf_observer.ko ]; then
    insmod /cf_observer.ko
else
    echo "[ERREUR] cf_observer.ko introuvable à la racine."
fi

#if [ -f /my_attack.ko ]; then
#    insmod /my_attack.ko
#else
#    echo "[ERREUR] my_attack.ko introuvable à la racine."
#fi
echo "======================================================="
...
EOF
            ...
            echo "-> Initramfs prêt et automatisé."
        else
            echo "Attention : Aucun binaire LKM trouvé dans $LKM_PATH."
        fi
...
echo "Lancement immédiat du Kernel $VERSION dans QEMU..."
